...
class ErrorGenerator(object):
    ...
    def __init__(
        ...
        self.error_run_length = (
            5,
            20,
        )  # higher = longer runs of errors when they appear
        self.error_run_density = (2, 7)  # higher = less error runs (must be int > 2)
        self.error_run_influence = (
            0.1  # closer to 0 = error run has MORE influence over error pos
        )
        self.error_run_smooth_iterations = (
            2  # smooth edges of error runs w moving average how many times?
        )
        self.error_run_filter_size = (
            3,
            10,
        ...
        if labeled_data is None:
            ...
            self.regression = LogisticRegression(
                max_iter=20, solver="sag", tol=0.03, verbose=1
            ).fit(X_one_hot, Y_labels)
        else:
            raise ValueError(
                "cannot supply training data with path to model in constructor"
            )
    ...
    def get_simple_synthetic_error_sequence(self, seq):
        ...
        if all(synthetic_error_alignment == self.match_idx):
            synthetic_error_alignment[
                len(synthetic_error_alignment) // 2
            ] = self.replace_idx
    ...
    def make_oscillator(self, length):
        ...
        error_run_density = np.random.randint(
            self.error_run_density[0], self.error_run_density[1]
        )
        ...
        error_run_filter_size = np.random.randint(
            self.error_run_filter_size[0], self.error_run_filter_size[1]
        )
        ...
        oscillator = (oscillator % error_run_density) == np.random.randint(
            error_run_density
        )
        ...
        for unused in range(error_run_smooth_iterations):
            oscillator = uniform_filter1d(
                oscillator, size=error_run_filter_size, mode="wrap"
            )
    ...
    def get_synthetic_error_sequence(self, seq):
        ...
        predictions_smooth_ind = expit(
            logit(predictions_smooth_ind) + logit(oscillator) + self.smoothing
        )

        # normalize remainder of predictions array
        target_sum = 1 - predictions_smooth_ind
        predictions_remainder = predictions_remainder / np.expand_dims(
            np.sum(predictions_remainder, 1), 1
        )
        predictions_remainder = predictions_remainder * np.expand_dims(target_sum, 1)

        predictions = np.insert(
            predictions_remainder, smooth_ind, predictions_smooth_ind, 1
        )
        ...
        labels = (
            predictions.cumsum(1) > np.random.rand(predictions.shape[0])[:, None]
        ).argmax(1)
