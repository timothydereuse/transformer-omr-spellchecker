...
import results_and_metrics as ttnm
...
criterion = torch.nn.BCEWithLogitsLoss(
    reduction="mean", pos_weight=torch.tensor(class_ratio)
)
...
for epoch in range(params.num_epochs):
    ...
    with torch.no_grad():
        val_loss, val_exs = tr_funcs.run_epoch(
            dloader=dloader_val, train=False, log_each_batch=False, **run_epoch_kwargs
        )
