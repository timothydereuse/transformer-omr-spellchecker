from data_augmentation.compare_felix_quartets import (
...
felix_dset_path = r"./processed_datasets/felix_quartets_bymeasure.h5"
supervised_targets_fname = r"./processed_datasets/supervised_omr_targets_bymeasure.h5"
error_generator_fname = (
    r"./processed_datasets/felix_quartets_error_model_bymeasure.joblib"
)
...
with h5py.File(felix_dset_path, "r") as f:
    ...
    correct_dset = [
        f["felix_correct"][x][:].astype(np.uint16)
        for x in correct_fnames
        if not "op80" in x
    ]
    error_dset = [f["felix_omr"][x][:].astype(np.uint16) for x in error_fnames]
    error_onepass_dset = [
        f["felix_onepass"][x][:].astype(np.uint16) for x in error_onepass_fnames
    ]
